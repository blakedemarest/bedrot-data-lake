...
def process_account_manual_persistent(playwright):
    print(
        "[INFO] Starting TikTok analytics extraction for PIG1987 (persistent context + real browser fingerprint)"
    )
    ...
    if os.path.exists(cookies_path) and not os.path.exists(marker_path):
        print(
            "[INFO] Importing cookies from tiktok_cookies_pig1987.json (one-time operation)..."
        )

...
def process_account_manual_persistent(playwright):
    print(
        "[INFO] Starting TikTok analytics extraction (persistent context + real browser fingerprint)"
    )
    ...
    page.goto(analytics_url)
    page.wait_for_url(
        analytics_url, timeout=120000
    )  # Wait up to 2 minutes for page to load
    ...
    if os.path.exists(cookies_path) and not os.path.exists(marker_path):
        print(
            "[INFO] Importing cookies from tiktok_cookies.json (one-time operation)..."
        )
        ...
        print(
            "[INFO] Cookies imported. Please refresh or re-navigate to TikTok if needed."
        )
        ...
        print(
            "[INFO] Cookie import already completed previously. Skipping cookie import."
        )

    print(
        "[ACTION] Please log in manually in the browser window if not already logged in."
    )
    print("[ACTION] After login, navigate to the analytics overview page:")
    print(
        "         https://www.tiktok.com/tiktokstudio/analytics/overview?from=dropdown_button&lang=en"
    )
    print(
        "[ACTION] The script will wait until you reach the correct analytics page, then continue."
    )

    # Wait until user lands anywhere in the analytics zone (any tab)
    analytics_prefix = "https://www.tiktok.com/tiktokstudio/analytics"
    print(
        f"[ACTION] Waiting for you to navigate to any analytics page starting with: {analytics_prefix}"
    )
    import time
    ...
    try:
        while waited < max_wait_seconds:
            ...
            if found:
                print(
                    "[INFO] Detected TikTok analytics page in one of the tracked tabs. Proceeding..."
                )
                break
            time.sleep(1)
            waited += 1
            ...
            print(
                f"[ERROR] Waited {max_wait_seconds} seconds but never saw analytics page in any tracked tab."
            )
        ...
        if not found_page:
            for p in tracked_pages:
                try:
                    ...
                    print(
                        "[INFO] Detected TikTok analytics page via wait_for_url in tracked tab. Proceeding..."
                    )
                    found_page = p
                    break
        ...
        filename = sanitize_filename(download.suggested_filename, "analytics.csv")

...
SESSION_DIR = str(
    Path(__file__).resolve().parent.parent.parent / ".playwright_dk_session"
)
# Output directory
...
async def main():
    async with async_playwright() as p:
        ...
        print(
            "[TOOLOST] Please log in to TooLost and complete any 2FA in the opened browser window."
        )
        ...
        try:
            await page.wait_for_selector(
                "main, .ant-layout-content, .analytics, .dashboard", timeout=10000
            )
            print("[TOOLOST] Analytics dashboard loaded!")
        except Exception:
            print(
                "[TOOLOST] Dashboard selector not found, but URL is correct. Proceeding after fallback delay."
            )
        ...
        try:
            await page.wait_for_selector(
                "[class*=ant-picker], [data-testid*=date], [role=button]",
                timeout=15000,
                state="visible",
            )
            ...
            try:
                await page.click(
                    "[class*=ant-picker], [data-testid*=date], [role=button]:has-text('Last 30 Days')",
                    timeout=5000,
                )
            except Exception:
                ...
                try:
                    ...
                    print(
                        "[TOOLOST] Could not find a clickable date picker. Skipping date selection."
                    )
            await asyncio.sleep(1)
            ...
            try:
                ...
                try:
                    ...
                    print(
                        "[TOOLOST] Could not find 'This Year' or 'Year' in dropdown. Please update selector if needed."
                    )
            await asyncio.sleep(2)
        except Exception:
            print(
                "[TOOLOST] Date picker not found after dashboard load. Skipping date selection."
            )
        ...
        if api_results["spotify"]:
            ...
            print(
                "Spotify API response not captured. Please check selectors or page behavior."
            )
        ...
        try:
            await page.click(
                "[role=button]:has-text('Spotify'), [data-testid*=platform], .ant-select-selector"
            )
            print("Platform dropdown opened.")
            # Wait for any dropdown option to appear
            await page.wait_for_selector(
                "div.d-flex.align-center.flex-row", timeout=5000
            )
            # Get all dropdown options
            option_elements = await page.query_selector_all(
                "div.d-flex.align-center.flex-row"
            )
        ...
        if api_results["apple"]:
            ...
            print(
                "[WARNING] Apple Music API response not captured after 60 seconds. Please check selectors or try again."
            )
        ...
        try:
            await page.wait_for_selector(
                "div.body-1.font-weight-bold.mb-1", timeout=10000
            )
        except Exception:
            print(
                "[TOOLOST] Notification blocks not found after navigation. Proceeding anyway."
            )
        await asyncio.sleep(2)
        ...
        while retry_count < max_retries and not found_report:
            ...
            for idx, text in enumerate(matching_blocks):
                if re.search(
                    r"Your \\(\d{2}-\d{4}\\) Sales report has been generated", text
                ):
                    latest_idx = idx
                    break
            if latest_idx is not None:
                print(
                    f"[TOOLOST] Found sales report notification: {matching_blocks[latest_idx]}"
                )
                ...
                if await attachment_buttons.count() > latest_idx:
                    ...
                    filename = sanitize_filename(
                        await download.suggested_filename(), "download.csv"
                    )
                    ...
                    print(
                        "[TOOLOST] Could not find matching attachment button for latest report."
                    )
                    break
            else:
                retry_count += 1
                if retry_count < max_retries:
                    print(
                        f"[TOOLOST] No sales report notification found! Retrying in 7 seconds... (Attempt {retry_count+1}/{max_retries})"
                    )
                    await asyncio.sleep(7)
                else:
                    print(
                        "[TOOLOST] No sales report notification found after multiple attempts!"
                    )

        print(
            "Data collection complete. You may now close the browser window to end the script."
        )
        await browser.close()
